import { TerminalLine } from "@/components/terminal-line";
import type { Layout, PileWindow } from "./before-after-data";

// Every length is in `em`, and the stage sets 1em from its own width, so the
// whole picture scales with one font-size instead of a breakpoint per width.

// Unfocused macOS windows show grey lights, which is both what a pile of
// buried terminals actually looks like and how the left half stays free of
// green.
const LIGHTS = "h-[0.7em] w-[0.7em] shrink-0 rounded-full bg-[#4a4a4a]";

// The seam between two overlapping dark windows comes from the ring, not from
// the page behind them, so the pile reads as separate windows in either theme.
const FRAME =
  "absolute overflow-hidden rounded-[0.6em] bg-[#1a1a1a] ring-1 ring-white/10 shadow-[0_1px_0_0_rgba(0,0,0,0.3),0_10px_24px_-10px_rgba(0,0,0,0.35)] dark:ring-white/[0.16] dark:shadow-[0_1px_0_0_rgba(0,0,0,0.8),0_10px_24px_-10px_rgba(0,0,0,0.85)]";

export function PileWindowCard({ win, layout }: { win: PileWindow; layout: Layout }) {
  const place = win[layout];
  const lines = place.lines ?? win.lines;
  return (
    <div
      className={FRAME}
      style={{
        left: `${place.x}em`,
        top: `${place.y}em`,
        width: `${place.w ?? win.w}em`,
        height: `${place.h ?? win.h}em`,
        rotate: `${place.r}deg`,
      }}
    >
      <div className="flex h-[2.6em] shrink-0 items-center gap-[0.55em] border-b border-[#2e2e2e] bg-[#252526] px-[0.8em]">
        <span className={LIGHTS} />
        <span className={LIGHTS} />
        <span className={LIGHTS} />
        {win.badge ? (
          // A port is a label, not a health signal, so it stays grey.
          <span className="shrink-0 rounded-[0.4em] bg-[#3a3a3a] px-[0.5em] py-[0.15em] font-mono text-[0.85em] font-semibold leading-[1.3] text-[#d4d4d4]">
            {win.badge}
          </span>
        ) : win.mark ? (
          <span className={`shrink-0 font-mono text-[0.9em] ${win.mark.c}`}>
            {win.mark.t}
          </span>
        ) : null}
        <span className="min-w-0 truncate font-mono text-[0.95em] text-[#919191]">
          {win.label}
        </span>
      </div>
      {/* A too-long line fades off the edge the way it would in a real pane;
          a hard cut is what makes a replica look like clip art. */}
      <div className="overflow-hidden px-[0.75em] py-[0.5em] font-mono [mask-image:linear-gradient(to_right,#000_93%,transparent)] [-webkit-mask-image:linear-gradient(to_right,#000_93%,transparent)]">
        {lines.map((l, i) => (
          <TerminalLine
            key={i}
            line={l}
            size={`text-[0.95em] ${win.session ? "leading-[1.5]" : "leading-[1.45]"}`}
            fallback="text-[#9a9a9a]"
            gapClass={win.session ? "mt-[1.5em]" : "mt-[1.45em]"}
          />
        ))}
      </div>
    </div>
  );
}
