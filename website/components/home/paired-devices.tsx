...
const CODEX_LINES: Line[] = [
  ...
  gap(s("• ", GREEN), s("Ran", BOLD), s(" npm test", TEXT)),
  line(s("  └ 14 passed (1.2s)", DIM)),
  gap(s("─ Worked for 1m 12s ────────────────────────────", DIM)),
  gap(s("• ", DIM), s("Added ?limit & ?cursor with stable ordering", TEXT)),
];

const ZSH_LINES: Line[] = [
  line(s("$ ", FAINT), s("ls", LIGHT)),
  line(
    s("README.md  package.json  ", LIGHT),
    s("routes", "font-semibold text-sky-400"),
    s("  ", LIGHT),
    s("src", "font-semibold text-sky-400"),
    s("  ", LIGHT),
    s("tests", "font-semibold text-sky-400"),
  ),
  line(s("$ ", FAINT), s("git status -s", LIGHT)),
  line(s(" M", RED), s(" src/auth/rateLimiter.ts", LIGHT)),
  line(s(" M", RED), s(" routes/users.ts", LIGHT)),
  line(s("$ ", FAINT), s("lpm start", LIGHT)),
  line(s("  ✓ api :4000 · web :3000", GREEN)),
];

const SESSIONS: Session[] = [
  { key: "claude", label: "claude", preamble: 3, lines: CLAUDE_LINES },
  { key: "codex", label: "codex", preamble: 6, lines: CODEX_LINES },
...
export function PairedDevices({
  ...
  useEffect(() => {
    ...
    goTab(2, 400);
    ZSH_LINES.forEach((zline, i) =>
      reveal(i + 1, zline.spans[0]?.t === "$ " ? 900 : 350),
    );
    hold(1800);
              ...
              <div className="flex h-[20rem] flex-col overflow-hidden rounded-t-md bg-[#1a1a1a]">
                ...
                <div className="flex min-h-0 flex-1">
                  ...
                  <div className="flex min-w-0 flex-1 flex-col">
                    ...
                    {/* Real Claude Code composer: ❯ prompt between ─ rules */}
                    {session.key === "claude" && (
