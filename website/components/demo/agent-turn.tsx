...
function ClaudeTurn({ query, steps, revealed, footer }: TurnProps) {
  return (
    <div className="mb-3">
      <div className="-mx-3 flex gap-[1ch] bg-white/[0.05] px-3 py-0.5">
        <span className="text-gray-400">{BRAND.claude.prompt}</span>
        <span className="whitespace-pre-wrap break-words text-white">{query}</span>
      </div>
          ...
          if (step.kind === "text") {
            ...
            return (
              <div key={i} className="flex gap-[1ch]">
                <span className={step.style === "muted" ? "text-gray-500" : "text-gray-100"}>
                  {BRAND.claude.bullet}
                </span>
                <span
                  className={`whitespace-pre-wrap break-words ${
                    step.style === "muted" ? "text-gray-500" : "text-gray-100"
                  }`}
                >
                  {step.text}
                </span>
            ...
            <div key={i}>
              <div className="flex gap-[1ch]">
                ...
                <span className="break-all">
                  ...
                  <span className="text-gray-400">({tool.arg})</span>
                </span>
              </div>
              <div className="flex gap-[2ch] pl-[2ch] text-gray-500">
                <span>⎿</span>
...
function CodexTurn({ query, steps, revealed, finished, footer }: TurnProps) {
  ...
  const flush = (key: string) => {
    ...
    blocks.push(
      <div key={key}>
        <div className="flex gap-[1ch]">
          <span className="text-gray-500">{BRAND.codex.bullet}</span>
          <span className="font-semibold text-gray-100">Explored</span>
        </div>
        {lines.map((detail, i) => {
          const [verb, target] = splitVerb(detail);
          return (
            <div key={i} className="flex gap-[1ch] pl-[2ch]">
              <span className="w-[1ch] shrink-0 text-gray-600">{i === 0 ? "└" : ""}</span>
              <span className="break-all">
                <span className="text-cyan-400">{verb}</span>{" "}
    ...
    if (tool.kind === "edited") {
      ...
      blocks.push(
        <div key={i} className="flex gap-[1ch]">
          <span className="text-gray-500">{BRAND.codex.bullet}</span>
          <span className="break-all">
            <span className="font-semibold text-gray-100">{verb}</span>{" "}
            <span className="text-gray-300">{target}</span>
            {tool.tag && <span className="ml-1 text-gray-500">({tool.tag})</span>}
          </span>
        </div>,
      );
      return;
    ...
    blocks.push(
      <div key={i}>
        <div className="flex gap-[1ch]">
          <span className="text-gray-500">{BRAND.codex.bullet}</span>
          <span className="break-all">
            <span className="font-semibold text-gray-100">Ran</span>{" "}
            <span className="text-gray-300">{tool.head}</span>
          </span>
        </div>
        <div className="flex gap-[1ch] pl-[2ch] text-gray-500">
  ...
  return (
    <div className="mb-3">
      <div className="flex gap-[1ch]">
        <span className="font-bold text-gray-500">{BRAND.codex.prompt}</span>
        <span className="whitespace-pre-wrap break-words text-gray-100">{query}</span>
      </div>
      <div className="mt-1.5 space-y-1.5">
        {blocks}
        {footer}
...
function Rule() {
  return <div className="-mx-3 border-t border-[#333333]" />;
}
