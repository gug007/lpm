...
### When to pick
Title "When to keep iTerm2 alone, and when to add lpm" (2026-09-23: "When each one is the right tool" was shared verbatim with `/vs/foreman` and `/vs/pm2`).
Five lpm bullets, iTerm2's four kept verbatim. The config bullet now reads "You want the service list drafted from your repo the moment you add it, instead of typed out by hand." (2026-09-23 — drafting is built in since commit `835443cb`; it used to name the agent CLI.)

### FAQ
Six items (§5). Items 3 and 5 are JSX and carry `answerText`.

### Related pages
Six: `/terminal-with-project-sidebar`, `/vs/tmux`, `/vs/cmux`, `/git-worktree-for-ai-agents`, `/config`, and (2026-09-23) `/mobile` — "The lpm Link app mirrors a tab running on the Mac, takes typing, and pings you when Claude Code or Codex needs an answer." — because the emulator shelf cites iTerm2 3.7's companion iOS app and the page never said lpm has one. `/best-terminal-for-mac` stays demoted to the in-body mention.
...
| A `.lpm.yml` in the repo travels with the branch and gives a teammate the same services, "alongside whatever lpm detected when they added it"; the same lines can live in your own project file instead *(ledger §5.5)* | QuickAnswer closing paragraph | `desktop/frontend/src-tauri/src/config.rs:1847-1856` (parses `<root>/.lpm.yml`), `:1861-1883` (merges its services + profiles **under** the personal project file); `projects_crud.rs:93` (adding the folder runs detection into that personal file with no `.lpm.yml` check) |
...
| "It writes the service list for you: add the folder and lpm reads package.json, a Procfile, a Gemfile, a compose file and the rest, then lists the services, with a port wherever the framework or the command names one — and Generate with AI hands a redraft to Claude Code, Codex, Gemini CLI or OpenCode." / WhenToPick "drafted from your repo the moment you add it" | FAQ 3 (and its JSON-LD), WhenToPick | `desktop/frontend/src-tauri/src/projects_crud.rs:48-67` (`services_for` writes `cmd`, `cwd`, `port`), `:93`, `:261`; `detect/mod.rs:111-130`; ports: `detect/mod.rs:273-291` (from the command), `detect/node.rs:14-29`, `detect/stacks.rs:51`, `detect/python.rs:50` (framework defaults); redraft: `aigen.rs:33-41`, `ConfigEditor.tsx:183`. 2026-09-23: the old sentence ("point Claude Code or Codex at the repo… the built-in scan offers the commands it finds as one-shot buttons") predated built-in detection. |
...
- QuickAnswer `CodeBlock` (`~/Projects/shop/.lpm.yml`) no longer carries `name:` / `root:` lines. A committed `.lpm.yml` contributes only `services` and `profiles` (`config.rs:1844-1856` `load_repo_yaml`, `:1861-1883` merge), and a machine-specific `root: ~/Projects/shop` in a file every teammate clones was misleading.
- EmulatorShelf iTerm2 card: "the deepest configuration surface on the shelf" was an unsourceable superlative (flagged by the previous pass). Now "free under GPLv2, scriptable from Python, and still moving: …" — GPLv2 is source 7, the Python API source 2, the 3.7 facts source 4.
- Verdict "Run both": "Nothing is captured, nothing is wrapped" → "— no container, no wrapper —". lpm does capture each service's output into a pane (`Pane.tsx:61`, `scrollback: 10000`), so "nothing is captured" was false as written; services start as a line typed into a login shell (`sessionpane.rs:58-61`, `sessions.rs:170-190`).
