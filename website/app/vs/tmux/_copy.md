...
Reviewed: `VS_REVIEWED` / `VS_REVIEWED_ISO` from `components/vs/reviewed.ts` (September 10, 2026 / 2026-09-10). No date is typed in this page.
...
| Drafts the whole config with your own agent CLI | Claude Code, Codex, Gemini CLI or OpenCode | ✗ |
...
| Your own shells survive a restart too | tabs come back, the shell restarts | ✓ |
...
| Remap every key and script custom layouts in a config file | three remappable hotkeys | via `.tmux.conf` |
...
| Service list drafted from your repo, then yours to edit (matrix row 2) | `desktop/frontend/src-tauri/src/aigen.rs:1-6` (shells out to the user's AI CLI and returns generated config text); `desktop/frontend/src/components/project-detail/useProjectSuggestions.ts:68-119` (repo scan). A new project starts from a placeholder, not a working stack: `desktop/frontend/src-tauri/src/projects_crud.rs:41` writes `echo 'configure me'` — which is why the row says "drafted … then yours to edit" and why no "zero config" claim survives on this page |
| Reads package.json scripts, Makefile targets, justfile recipes and compose files (matrix row 3) | `useProjectSuggestions.ts:72-74` (`package.json`, `Makefile`/`makefile`, `justfile`/`Justfile`/`.justfile`), `:92-97` (`docker-compose.yml\|yaml`, `compose.yml\|yaml`), `:104-118` (parsed into suggestions) |
| Drafts the whole config with Claude Code, Codex, Gemini CLI or OpenCode (matrix row 4) | `desktop/frontend/src-tauri/src/aigen.rs:1-2`, `:34-41` (`check_aicl_is` probes `claude`, `codex`, `gemini`, `opencode` on PATH), `:43-45` `is_supported_cli` |
...
| Your own shells: tabs come back, the shell restarts (matrix row 9) | `desktop/frontend/src/paneTree.ts:24-26` (`id` is "a live PTY id regenerated on every restart"; the tab is what persists), `:44-46` (only a peer-owned tab re-adopts a running session); `desktop/frontend/src-tauri/src/pty.rs:1-9` (terminals are in-process PTYs with reader threads, no separate holder) |
...
| lpm will not read a Procfile (RelatedPages `/vs/overmind` card) | `grep -rn Procfile desktop cli` returns zero hits |
...
| Forward remote ports to localhost from the app (FAQ 5) | `desktop/frontend/src-tauri/src/portforward.rs:1-8` — manual add/remove/list over long-lived `ssh -N -L` children. The suggestion poller is deferred, so no auto-suggested forward is claimed anywhere on this page |
| The remote box needs nothing but git — no tmux server there either (FAQ 5) | `desktop/frontend/src-tauri/src/peerssh.rs:48-53` ("tmux used to be … but a host now runs its services in lpm's own session layer"; `REQUIRED_TOOLS: [&str; 1] = ["git"]`) |
| Three remappable shortcuts, no layout scripting (matrix row 15, Jobs card 3) | `desktop/frontend/src/hotkeys.ts:3-33` (`HotkeyId` is exactly `toggleAgentOverview`, `tabSwitchNext`, `tabSwitchPrev`, each with one default) |
