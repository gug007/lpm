...
| `{ "t": "createProject", "name": "<name>", "root": "<abs or ~ path>" }` | `{ "t": "createProject", "ok": true, "name": "<landed name>", "existing": false }` / `{ "ok": false, "name": "<name>", "error": "…" }` — creates the project `name` rooted at `root`, creating the folder if missing (so it works for both a new project and importing an existing folder) and seeding a placeholder `dev` service. `name` is only a wish: when it is already taken (possibly by a project shown under another label) the project lands as `<name>-2`, `<name>-3`, …, and the reply's `name` is the one it landed under. When `root` is already a project's root, nothing is written and the reply carries that project's `name` with `existing: true`. Rejects an invalid name. Inline; emits `projects-changed` |
