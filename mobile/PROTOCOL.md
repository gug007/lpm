...
| `{ "t": "gitDiffs", "project": "<name>", "files": [{ "path": "<file>", "status": "<status>"? }…], "reqId": <any>? }` | `{ "t": "gitDiffs", "project": "<name>", "ok": true, "diffs": { "<path>": { "original": "…", "modified": "…", "binary": bool, "tooLarge": bool }… }, "reqId": <any>? }` / `{ "ok": false, "error": "…" }` — batched **full original/modified contents** (HEAD vs working tree) for several files at once, for a client that renders the diff from two full buffers (the desktop peer's Monaco review pane) rather than a unified string. Each side beyond ~4 MB returns `tooLarge:true` with empty content (like `binary`). Pass `reqId` (string/number) and it is echoed verbatim so concurrent requests correlate by id. This is the counterpart to `gitDiff` (unified string, for the phone); both read the same working-tree state |
