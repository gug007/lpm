// v2-shaped runtime shim, backed by @tauri-apps/api (Tauri 2). Keeps the
// frontend files that import from this path working unchanged after the
// Wails -> Tauri migration.
...
// onDragDropEvent; re-publish it as the `wails:*` CustomEvents the drag
  ...
  void getCurrentWebview().onDragDropEvent((event) => {
    ...
    switch (p.type) {
      case "enter": {
        ...
        window.dispatchEvent(new CustomEvent("wails:handleDragEnter"));
        window.dispatchEvent(new CustomEvent("wails:handleDragOver", { detail: [x, y] }));
        break;
      }
      case "over": {
        const { x, y } = toLogical(p.position);
      ...
      case "leave":
        window.dispatchEvent(new CustomEvent("wails:handleDragLeave"));
        break;
      case "drop": {
        const { x, y } = toLogical(p.position);
        window.dispatchEvent(new CustomEvent("wails:filesDropped"));
...
// custom property `--wails-draggable: drag` (class `.wails-drag`) and excludes
// interactive bits with `--wails-draggable: no-drag`. Tauri has no native
// equivalent, so translate: on primary-button mousedown, if the target's
// computed `--wails-draggable` resolves to "drag", start an OS window drag
