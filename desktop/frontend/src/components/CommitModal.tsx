...
export function CommitModal({
  ...
  const [busy, setBusy] = useState<false | "commit" | "push" | "discard">(
    false,
  );
  const [loading, setLoading] = useState(false);
  const [confirmDiscardPath, setConfirmDiscardPath] = useState<string | null>(
    null,
  );
      ...
      >
        ...
        <div className="flex items-center justify-between border-t border-[var(--border)] px-5 py-3">
          <span className="flex items-center gap-3 text-[11px] text-[var(--text-muted)]">
            ...
            {ai.anyAvailable && (
              <Tooltip
                content="Auto-generate commit message on open"
                side="top"
                align="start"
              >
