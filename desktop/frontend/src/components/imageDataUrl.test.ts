...
const mocks = vi.hoisted(() => ({
  readFile: vi.fn(() => Promise.resolve({ mimeType: "image/png", data: "AAA" })),
}));
...
describe("loadImageDataUrl", () => {
  ...
  it("reads a peer path as a marked root so the read is routed to the host", async () => {
    ...
    expect(mocks.readFile).toHaveBeenCalledWith(`/@peer-${SLUG}/tmp/peer-1.png`);
  });
  ...
  it("keeps the same path on different machines apart in the cache", async () => {
    ...
    await expect(loadImageDataUrl("/tmp/shared.png")).resolves.toBe("data:image/png;base64,AAA");
