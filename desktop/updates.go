...
func appBundlePath() (string, error) {
	...
	// Executable lives at e.g. /Applications/lpm.app/Contents/MacOS/lpm.
...
func (a *App) InstallUpdate() error {
	...
	// Defers won't run after os.Exit, so cleanup must happen here.
	exec.Command("hdiutil", "detach", mountPoint, "-quiet").Run()
	os.Remove(dmgPath)
	os.Remove(mountPoint)

	// Wait-then-launch via a detached process avoids two instances fighting
	// over the same Wails port / dock icon.
	script := fmt.Sprintf(
	...
	_ = a.ServiceShutdown()
	forceTerminate()
	// Fallback if the Cocoa run-loop can't process terminate in time.
